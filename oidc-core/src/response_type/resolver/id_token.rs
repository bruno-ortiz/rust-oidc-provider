...
impl ResponseTypeResolver for IDTokenResolver<'_> {
    fn resolve(&self, context: &OpenIDContext) -> Result<AuthorisationResponse, AuthorisationError> {
        let state = context.request.state.as_ref().ok_or(AuthorisationError::MissingState)?;
        let signing_key = context.configuration.signing_key().ok_or(AuthorisationError::SigningKeyNotConfigured)?;
        let s_hash = state.hash(signing_key)
            .map_err(|source| {
                AuthorisationError::HashingErr { prop: "state".to_owned(), source }
            })?;
